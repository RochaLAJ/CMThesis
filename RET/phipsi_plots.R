...
colnames(list_of_files[[1]]) = c("Frames","Phi215", "Psi215", "Phi216", "Psi216")
colnames(list_of_files[[2]]) = c("Frames","Phi215", "Psi215", "Phi216", "Psi216")
colnames(list_of_files[[3]]) = c("Frames","Phi215", "Psi215", "Phi216", "Psi216")

create_plot <- function(file, colour) {
  ggplot() +
...
plots_list <- mapply(create_plot, file = list_of_files, colour = c("darkgreen", "orange", "red"), SIMPLIFY = FALSE)

combined_plots <- plot_grid(plotlist = plots_list, nrow = 1, ncol = 3, rel_heights = c(1,1))

concise_title <- ggdraw() +
  draw_label("PhiPsi Relation for Wildtype, M918V and M918T", size = 21)


final_plot <- plot_grid(concise_title, combined_plots, nrow = 2, rel_heights = c(0.1, 1))

print(final_plot)
ggsave("PhiPsi918.png", dpi=600, width=10, height=7, bg="white")
